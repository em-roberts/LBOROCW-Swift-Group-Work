...
public var g1 = Array(repeating: Grid(hx: [0.0], chxh: [0.0], chxe: [0.0],
...
public func tfsfUpdate(_ a: Grid) -> Void {
    ...
    if Type() == GridType.tmZGrid.rawValue {
        ...
        for nn in firstY ..< lastY {
            g.hy[mm * SizeY() + nn] -= Chyh(mm, nn) * Ez1(mm + 1, nn)
        }
        mm = lastX
        for nn in firstY ..< lastY {
            g.hy[mm * SizeY() + nn] += Chye(mm, nn) * Ez1(mm, nn)
        }
        
        nn = firstY - 1
        for mm in firstX ..< lastX {
            g.hx[mm * (SizeY() - 1) + nn] += Chxe(mm, nn) * Ez1(mm, nn)
        }
        nn = lastY
        for mm in firstX ..< lastX {
            g.hx[mm * (SizeY() - 1) + nn] -= Chxe(mm, nn) * Ez1(mm, nn)
        ...
        mm = firstX - 1
        for nn in firstY ..< lastY {
            g.hz[mm * (SizeY() - 1) + nn] += Chze(mm, nn) * Ey1(mm + 1, nn)
        }
        mm = lastX
        for nn in firstY ..< lastY {
            g.hz[mm * (SizeY() - 1) + nn] -= Chze(mm, nn) * Ey1(mm, nn)
        }
    }
    ...
    if Type() == GridType.tmZGrid.rawValue {
        for nn in 0 ..< SizeY() {
            g1[nn].ez[0] = ezInc(g1[nn].time, location: 0.0)
            g1[nn].time += 1
        }
        
        mm = firstX
        for nn in firstY ..< lastY {
            g.ez[mm * SizeY() + nn] -= Cezh(mm, nn) * Hy1(mm - 1, nn)
            
        }
        mm = lastX
        for nn in firstY ..< lastY {
            g.ez[mm * SizeY() + nn] += Cezh(mm, nn) * Hy1(mm, nn)
        }
    } else {
        for nn in 0 ..< SizeY() {
            g1[nn].ey[0] = ezInc(g1[nn].time, location: 0.0)
            g1[nn].time += 1
        }
        
        nn = firstY
        for mm in firstX ..< lastX {
            g.ex[mm * SizeY() + nn] -= Cexh(mm, nn) * Hz1(mm, nn)
        }
        nn = lastX
        for mm in firstX ..< lastX {
            g.ex[mm * SizeY() + nn] += Cexh(mm, nn) * Hz1(mm, nn)
        ...
        mm = firstX
        for nn in firstY ..< lastY {
            g.ey[mm * (SizeY() - 1) + nn] += Ceyh(mm, nn) * Hz1(mm - 1, nn)
        }
        mm = lastX
        for nn in firstY ..< lastY {
            g.ey[mm * (SizeY() - 1) + nn] -= Ceyh(mm, nn) * Hz1(mm, nn)
        }
    }
    return
}
